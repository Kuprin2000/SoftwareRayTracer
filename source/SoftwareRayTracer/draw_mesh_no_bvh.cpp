...
namespace
{
	...
	_NODISCARD bool ray_intersects_mesh(const cmesh4::SimpleMesh& mesh, const float3& ray_point,
	...
	void calc_mesh_color(const AppData& app_data, const cmesh4::SimpleMesh& mesh, const float3& point, const float3& normal, uint32_t& result)
	{
		float dummy_t;
		float3 dummy_point;
		float3 dummy_normal;
		...
		if (!ray_intersects_mesh(mesh, point, app_data.light_dir, dummy_t, dummy_point, dummy_normal))
		{
	...
	void calc_plane_color(const AppData& app_data, const cmesh4::SimpleMesh& mesh,
		const float3& prev_ray_dir, const float3& point, const float3& normal, uint32_t& result)
	{
		...
		const bool intersects_mesh =
			ray_intersects_mesh(mesh, point, new_ray_dir, mesh_intersection_t, mesh_intersection_point, mesh_intersection_normal);

		if (intersects_mesh)
		{
			mesh_intersection_point += 0.01f * mesh_intersection_normal;
			calc_mesh_color(app_data, mesh, mesh_intersection_point, mesh_intersection_normal, result);
		}
	}
}
...
#pragma omp parallel for
		for (int i = 0; i < app_data.m_height; ++i)
		{
			for (int j = 0; j < app_data.m_width; ++j)
			{
				...
				float3 mesh_intersection_normal;
				const bool intersects_mesh =
					ray_intersects_mesh(mesh, camera_pos, ray_dir, mesh_intersection_t, mesh_intersection_point, mesh_intersection_normal);
				...
				if (need_draw_plane)
				{
					...
					calc_plane_color(app_data, mesh, ray_dir, plane_intersection_point, plane_normal, frame_buffer[dst_offset]);
				}
				else
				{
					mesh_intersection_point += 0.01f * mesh_intersection_normal;
					const uint32_t dst_offset = i * app_data.m_width + j;
					calc_mesh_color(app_data, mesh, mesh_intersection_point, mesh_intersection_normal, frame_buffer[dst_offset]);
				}
			}
		}
	}
